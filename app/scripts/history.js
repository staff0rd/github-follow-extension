...
function follow(sha, fileName, ownerRepo) {
	...
	get(url).done(function (data) {
		...
		var template = ['<div id="github-follow" class="commit-group-title"><span class="octicon octicon-git-commit"></span>', '<strong>Renamed</strong> to :fileName <a class="btn right" href=":link">Follow</a></div>'].join('').replace(':fileName', fileName).replace(':link', link);
